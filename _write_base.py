# -*- coding: utf-8 -*-
"""Apply the 3-part fix for multi-timeframe data delivery."""
from pathlib import Path

# ── Fix 1: Add required_timeframes to StrategyMeta ────────────────────────────
base_p = Path("strategies/base.py")
base_t = base_p.read_text(encoding="utf-8")

old_meta = """    typical_timeframes: list[str] = field(default_factory=list)
    default_symbol: str = ""

@dataclass
class Signal:"""

new_meta = """    typical_timeframes: list[str] = field(default_factory=list)
    default_symbol: str = ""
    required_timeframes: list[str] = field(default_factory=list)  # additional TFs beyond self.timeframe

@dataclass
class Signal:"""

assert old_meta in base_t, "StrategyMeta block not found in base.py"
base_t = base_t.replace(old_meta, new_meta, 1)
base_p.write_text(base_t, encoding="utf-8")
print("Fix 1: StrategyMeta.required_timeframes added")

# ── Fix 2: BacktestEngine multi-TF resampling ──────────────────────────────────
engine_p = Path("quant/backtest_engine.py")
engine_t = engine_p.read_text(encoding="utf-8")

# Add _TF_TO_PANDAS_RULE and _resample helper right after class close marker
insert_after = '# -- Module-level helpers ------------------------------------------------------'
helper_block = '''

_TF_TO_PANDAS_RULE: dict[str, str] = {
    "M1": "1T", "M5": "5T", "M15": "15T", "M30": "30T",
    "H1": "1H", "H4": "4H", "D1": "1D",
}


def _resample(df: pd.DataFrame, target_tf: str) -> pd.DataFrame | None:
    """Resample a lower-TF DataFrame up to target_tf OHLCV bars."""
    rule = _TF_TO_PANDAS_RULE.get(target_tf.upper())
    if rule is None:
        log.warning("_resample: unknown target TF %r", target_tf)
        return None
    needed = {
        "M1": 10, "M5": 10, "M15": 10, "M30": 15,
        "H1": 30, "H4": 30, "D1": 60,
    }.get(target_tf.upper(), 30)
    if len(df) < needed:
        log.debug("_resample: only %d source bars for %s (need %d)", len(df), target_tf, needed)
        return None
    try:
        out = df.resample(rule).agg(
            open=("open", "first"),
            high=("high", "max"),
            low=("low", "min"),
            close=("close", "last"),
            tick_volume=("tick_volume", "sum"),
            volume=("volume", "sum"),
            spread=("spread", "mean"),
        ).dropna(subset=["close"])
        return out if not out.empty else None
    except Exception as exc:
        log.warning("_resample failed %s: %s", target_tf, exc)
        return None


'''

assert insert_after in engine_t, f"Insert marker not found in backtest_engine.py"
engine_t = engine_t.replace(insert_after, helper_block + "\n" + insert_after, 1)

# Now add the multi-TF data building inside run(), right before generate_signal call
old_sig_block = """            # - Generate signal — ONLY past data visible
            # CLAUDE.md rule: slice_data = {tf: df.iloc[:i+1]}
            slice_data = {timeframe: df.iloc[: i + 1].copy()}

            signal = None"""

new_sig_block = """            # -- Build multi-TF data dict (primary + any required trend TFs)
            # MomentumReversion needs H1 alongside M15, etc.
            slice_data: dict[str, pd.DataFrame] = {timeframe: df.iloc[: i + 1].copy()}
            req_tfs = getattr(getattr(strategy, 'meta', None), 'required_timeframes', []) or []
            for req_tf in req_tfs:
                if req_tf == timeframe:
                    continue  # already have primary TF
                if req_tf in slice_data:
                    continue  # already added
                resampled = _resample(df.iloc[: i + 1], req_tf)
                if resampled is not None:
                    slice_data[req_tf] = resampled
                    log.debug(
                        "BacktestEngine: added %s data (%d bars) for %s",
                        req_tf, len(resampled), strategy_class.__name__,
                    )
                else:
                    log.debug(
                        "BacktestEngine: could not build %s for %s at bar %d",
                        req_tf, strategy_class.__name__, i,
                    )

            # -- Generate signal — ONLY past data visible
            # CLAUDE.md rule: slice_data = {tf: df.iloc[:i+1]}
            signal = None"""

assert old_sig_block in engine_t, "Signal block not found in backtest_engine.py"
engine_t = engine_t.replace(old_sig_block, new_sig_block, 1)
engine_p.write_text(engine_t, encoding="utf-8")
print("Fix 2: BacktestEngine multi-TF data builder added")

# ── Fix 3: Add required_timeframes to MomentumReversion.meta ──────────────────
mr_p = Path("strategies/momentum_reversion.py")
mr_t = mr_p.read_text(encoding="utf-8")

old_meta_mr = '''    name: str = "momentum_reversion"
    label: str = "Momentum Reversion"
    description: str = ""
    asset_class: str = "crypto"
    version: str = "1.0.0"
    typical_timeframes: list[str] = field(default_factory=list)
    default_symbol: str = ""'''

new_meta_mr = '''    name: str = "momentum_reversion"
    label: str = "Momentum Reversion"
    description: str = ""
    asset_class: str = "crypto"
    version: str = "1.0.0"
    typical_timeframes: list[str] = field(default_factory=list)
    default_symbol: str = ""
    required_timeframes: list[str] = field(default_factory=lambda: ["H1"])'''

assert old_meta_mr in mr_t, "MomentumReversion meta block not found"
mr_t = mr_t.replace(old_meta_mr, new_meta_mr, 1)
mr_p.write_text(mr_t, encoding="utf-8")
print("Fix 3: MomentumReversion.required_timeframes=['H1'] added")

# ── Fix 4: Live engine _phase_scan — bundle multi-TF data ─────────────────────
loop_p = Path("engine/engine_loop.py")
loop_t = loop_p.read_text(encoding="utf-8")

old_scan_call = """        instance = cls(default_symbol=symbol, params=params)
        try:
            signal = instance.generate_signal({timeframe: df})"""

new_scan_call = """        instance = cls(default_symbol=symbol, params=params)
        # Build multi-TF data bundle (primary + any required secondary TFs)
        tf_data: dict[str, pd.DataFrame] = {timeframe: df}
        req = getattr(getattr(instance, "meta", None), "required_timeframes", []) or []
        for extra_tf in req:
            if extra_tf == timeframe or extra_tf in tf_data:
                continue
            extra_df = _fetch_ohlcv_mt5(symbol, extra_tf, strat.name)
            if extra_df is not None and not extra_df.empty:
                tf_data[extra_tf] = extra_df
                log.debug(
                    "Engine: loaded %s bars for %s %s (%s)",
                    len(extra_df), symbol, extra_tf, strat.name,
                )
        try:
            signal = instance.generate_signal(tf_data)"""

assert old_scan_call in loop_t, f"_phase_scan call block not found in engine_loop.py — diff=\n{old_scan_call!r}"
loop_t = loop_t.replace(old_scan_call, new_scan_call, 1)
loop_p.write_text(loop_t, encoding="utf-8")
print("Fix 4: Live engine _phase_scan bundles multi-TF data")

print("\nAll fixes applied.")
