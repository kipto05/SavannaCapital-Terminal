...
class BacktestEngine:
    ...
    def run(
        ...
        for i in range(self.warmup, n - 1):
            ...
            dd = (peak - equity[-1]) / peak if peak > 0 else 0.0
            drawdown.append(dd)
            continue  # next bar — trade carries over; final-close happens after the loop
...
def symbol_for_sl_tp(trade: TradeResult) -> str:
    ...
    # Tag format is "StrategyName.SIDE" — no symbol info.
    # Caller should pass symbol explicitly; this is a best-effort fallback.
    return ""
